let companyController = function(
  ...
  $scope.taxTypes = ["MONTHLY", "YEARLY", "QUARTERLY", "HALFYEARLY"];
  $scope.monthDays = [
    1,
    2,
    3,
  ...
  $scope.getUserDetails = function() {
    ...
    return userServices
      .get($rootScope.basicInfo.uniqueName)
      .then($scope.getUserDetailSuccess, $scope.getUserDetailFailure);
  };
  ...
  $scope.goToCompany = function(data, index, type) {
    ...
    if (
      !_.isNull(contactnumber) &&
      !_.isEmpty(contactnumber) &&
      !_.isUndefined(contactnumber) &&
      contactnumber.match("-")
    ) {
  ...
  $scope.updateCompanyInfo = function(data) {
    ...
    return companyServices
      .update(abc)
      .then($scope.updtCompanySuccess, $scope.updtCompanyFailure);
  };
  ...
  $scope.getCountry = val =>
    locationService
      .searchCountry(val)
      .then($scope.onGetCountrySuccess, $scope.onGetCountryFailure);
  ...
  $scope.getState = val =>
    locationService
      .searchState(val, $rootScope.selectedCompany.country)
      .then($scope.onGetStateSuccess, $scope.onGetStateFailure);

  $scope.onGetStateSuccess = function(data) {
    let filterThis = data.results.filter(i =>
      _.contains(i.types, "administrative_area_level_1")
    );
    return filterThis.map(item => item.address_components[0].long_name);
  };
  ...
  $scope.getCity = val =>
    locationService
      .searchCity(val, $rootScope.selectedCompany.state)
      .then($scope.onGetCitySuccess, $scope.onGetCityFailure);
  ...
  $scope.getCurrencyList = function() {
    ...
    if (
      !_.isNull(currencyListt) &&
      !_.isEmpty(currencyListt) &&
      !_.isUndefined(currencyListt)
    ) {
      return ($scope.currencyList = currencyListt);
    } else {
      return currencyService.getList(
        $scope.getCurrencyListSuccess,
  ...
  $scope.updateUserRole = function(role, userEmail) {
    ...
    return companyServices
      .share($rootScope.selectedCompany.uniqueName, sData)
      .then($scope.onShareCompanySuccess, $scope.onShareCompanyFailure);
  };
  ...
  $scope.shareCompanyWithUser = function() {
    ...
    return companyServices
      .share($rootScope.selectedCompany.uniqueName, $scope.shareRequest)
      .then($scope.onShareCompanySuccess, $scope.onShareCompanyFailure);
  };
  ...
  $scope.getSharedUserList = uniqueName =>
    companyServices
      .shredList(uniqueName)
      .then($scope.getSharedUserListSuccess, $scope.getSharedUserListFailure);
  ...
  $scope.unSharedUser = function(uNqame, id) {
    ...
    return companyServices
      .unSharedComp($rootScope.selectedCompany.uniqueName, data)
      .then($scope.unSharedCompSuccess, $scope.unSharedCompFailure);
  };
  ...
  $scope.exceptOwnEmail = email =>
    $rootScope.basicInfo.email !== email.userEmail;

  $scope.getUploadsList = () =>
    companyServices
      .getUploadsList($rootScope.selectedCompany.uniqueName)
      .then($scope.getUploadsListSuccess, $scope.getUploadsListFailure);
  ...
  $scope.$watch("selectedCompany.companySubscription.autoDeduct", function(
    newVal,
    oldVal
  ) {
  ...
  $scope.pageChangedComp = function(data) {
    ...
    return companyServices
      .getCompTrans(obj)
      .then($scope.pageChangedCompSuccess, $scope.pageChangedCompFailure);
  };

  $scope.pageChangedCompSuccess = function(res) {
    $scope.compTransData.paymentDetail = $scope.compTransData.paymentDetail.concat(
      res.body.paymentDetail
    );
  ...
  $scope.getCompanyTransactions = function() {
    ...
    return companyServices
      .getCompTrans(obj)
      .then(
        $scope.getCompanyTransactionsSuccess,
        $scope.getCompanyTransactionsFailure
      );
  };
  ...
  $scope.updateCompSubs = function(resObj) {
    ...
    return companyServices
      .updtCompSubs(data)
      .then($scope.updateCompSubsSuccess, $scope.updateCompSubsFailure);
  };
  ...
  $scope.getWltBal = () =>
    userServices
      .getWltBal($rootScope.basicInfo.uniqueName)
      .then($scope.getWltBalSuccess, $scope.getWltBalFailure);

  $scope.getWltBalSuccess = function(res) {
    $scope.disableRazorPay = false;
    let avlB = Number(res.body.availableCredit);
    let invB = Number(
      $rootScope.selectedCompany.companySubscription.billAmount
    );
    if (avlB >= invB) {
      $scope.showPayOptns = false;
  ...
  $scope.deductSubsViaWallet = function(num) {
    ...
    return companyServices
      .payBillViaWallet(obj)
      .then($scope.subsViaWltSuccess, $scope.subsWltFailure);
  };
  ...
  $scope.addBalViaDirectCoupon = function() {
    ...
    return userServices
      .addBalInWallet(obj)
      .then($scope.addBalRzrSuccess, $scope.addBalRzrFailure);
  };
  ...
  $scope.addBalRzrSuccess = function(res) {
    if ($scope.isHaveCoupon && !_.isEmpty($scope.coupRes)) {
      if ($scope.coupRes.type === "balance_add" && $scope.coupRes.extra) {
        ...
        $rootScope.basicInfo.availableCredit += Number(
          $scope.coupRes.maxAmount
        );
      } else {
        $rootScope.basicInfo.availableCredit += Number($scope.amount);
      }
    ...
    if ($scope.wlt.status) {
      ...
      if (
        $rootScope.basicInfo.availableCredit >=
        $rootScope.selectedCompany.companySubscription.billAmount
      ) {
        return $scope.deductSubsViaWallet(
          Number($rootScope.selectedCompany.companySubscription.billAmount)
        );
      } else {
        $scope.amount -= Number($scope.coupRes.maxAmount);
        $scope.directPay = false;
        $scope.disableRazorPay = false;
        return $scope.payAlert.push({
          msg: `Coupon is redeemed. But for complete subscription, you have to add Rs. ${$scope.amount} more in your wallet.`
        });
      }
    }
  };
  ...
  $scope.redeemCoupon = code =>
    couponServices
      .couponDetail(code)
      .then($scope.redeemCouponSuccess, $scope.redeemCouponFailure);
  ...
  $scope.redeemCouponSuccess = function(res) {
    ...
    $scope.payAlert.push({
      type: "success",
      msg: `Coupon code is redeemed. You can get a max discount of Rs. ${$scope
        .coupRes.maxAmount}`
    });
  ...
  $scope.checkDiffAndAlert = function(type) {
    ...
    switch (type) {
      case "cashback_discount":
        $scope.disableRazorPay = false;
        return $scope.payAlert.push({
          msg: `Your cashback amount will be credited in your account withing 48 hours after payment has been done. Your will get a refund of Rs. ${$scope.cbDiscount}`
        });
      case "cashback":
        if ($scope.amount < $scope.coupRes.value) {
          $scope.disableRazorPay = true;
          return $scope.payAlert.push({
            msg: `Your coupon is redeemed but to avail coupon, You need to make a payment of Rs. ${$scope
              .coupRes.value}`
          });
        } else {
          $scope.disableRazorPay = false;
          return $scope.payAlert.push({
            type: "success",
            msg: `Your cashback amount will be credited in your account withing 48 hours after payment has been done. Your will get a refund of Rs. ${$scope
              .coupRes.value}`
          });
        }
      ...
      case "discount":
        ...
        if (diff < 100) {
          $scope.disableRazorPay = true;
          return $scope.payAlert.push({
            msg: `After discount amount cannot be less than 100 Rs. To avail coupon you have to add more money. Currently payable amount is Rs. ${diff}`
          });
        } else {
          $scope.disableRazorPay = false;
          return $scope.payAlert.push({
            type: "success",
            msg: `Hurray you have availed a discount of Rs. ${$scope.discount}. Now payable amount is Rs. ${diff}`
          });
        }
      case "discount_amount":
  ...
  $scope.openFixUploadIssues = function(data) {
    ...
    if (data.isGroupConflict) {
      $scope.fixUploadData.groupConflicts = _.reject(
        $scope.fixUploadData.groupConflicts,
        grpC =>
          _.some(
            $scope.flattenGroupList,
            grp => grp.uniqueName === grpC.uniqueName
          )
      );
    }
    if (data.isAccountConflict) {
      $scope.fixUploadData.accountConflicts = _.reject(
        $scope.fixUploadData.accountConflicts,
        acObj =>
          _.some(
            $scope.fltAccntListPaginated,
            ac =>
              ac.uniqueName === acObj.uniqueName ||
              ac.mergedAccounts.indexOf(acObj.uniqueName) !== -1
          )
      );
    }
    $scope.modal = {};
  ...
  $scope.fixMoveGroup = function(group, togroup) {
    if (_.isObject(togroup)) {
      ...
      return groupService
        .create($rootScope.selectedCompany.uniqueName, body)
        .then($scope.fixMoveGroupSuccess, $scope.fixMoveGroupFailure);
    } else {
  ...
  $scope.fixMoveGroupSuccess = function(res) {
    ...
    $scope.fixUploadData.groupConflicts = _.reject(
      $scope.fixUploadData.groupConflicts,
      grp => grp.uniqueName === res.body.uniqueName
    );
  ...
  $scope.fixMoveAc = function(ac, group) {
    if (_.isObject(group)) {
      ...
      return accountService
        .createAc(unqNamesObj, ac)
        .then($scope.fixMoveAcSuccess, $scope.fixMoveAcFailure);
    } else {
  ...
  $scope.fixMoveAcSuccess = function(res) {
    ...
    $scope.fixUploadData.accountConflicts = _.reject(
      $scope.fixUploadData.accountConflicts,
      acObj => acObj.uniqueName === res.body.uniqueName
    );
    return $scope.getGroups();
  };
  ...
  $scope.retryUpload = function(data) {
    ...
    return companyServices
      .retryXml($rootScope.selectedCompany.uniqueName, data)
      .then($scope.retryUploadSuccess, $scope.retryUploadFailure);
  };
  ...
  $scope.getTax = function() {
    ...
    if ($rootScope.canUpdate && $rootScope.canDelete) {
      return companyServices
        .getTax($rootScope.selectedCompany.uniqueName)
        .then($scope.getTaxSuccess, $scope.getTaxFailure);
    }
  };
  ...
  $scope.getTaxSuccess = function(res) {
    if (res.body.length === 0) {
      ...
      return _.each(res.body, function(obj) {
        ...
        obj.hasLinkedAcc = _.find(
          $scope.fltAccntListPaginated,
          acc => acc.uniqueName === obj.account.uniqueName
        );
  ...
  $scope.addNewTax = function(newTax) {
    ...
    return companyServices
      .addTax($rootScope.selectedCompany.uniqueName, newTax)
      .then($scope.addNewTaxSuccess, $scope.addNewTaxFailure);
  };
  ...
  $scope.deleteTaxconfirmation = data =>
    modalService
      ...
      .then(function() {
        ...
        return companyServices
          .deleteTax(reqParam)
          .then($scope.deleteTaxSuccess, $scope.deleteTaxFailure);
      });
  ...
  $scope.updateTax = function(item) {
    ...
    _.each($scope.taxDetail_1, (tax_1, idx) =>
      _.each(item.taxDetail, function(tax, index) {
        if (
          tax.taxValue.toString() !== tax_1.taxValue.toString() &&
          idx === index
        ) {
    ...
    _.each(
      newTax.taxDetail,
      detail => (detail.value = detail.taxValue.toString())
    );
    ...
    if ($scope.taxValueUpdated) {
      ...
      companyServices
        .editTax(reqParam, newTax)
        .then($scope.updateTaxSuccess, $scope.updateTaxFailure);
  ...
  $scope.getKeys = function() {
    companyServices
      .getSmsKey($rootScope.selectedCompany.uniqueName)
      .then($scope.getSmsKeySuccess, $scope.getSmsKeyFailure);
    return companyServices
      .getEmailKey($rootScope.selectedCompany.uniqueName)
      .then($scope.getEmailKeySuccess, $scope.getEmailKeyFailure);
  };
  ...
  $scope.saveMsg91 = function() {
    ...
    if ($scope.msg91.authKey.length > 0 && $scope.msg91.senderId.length > 0) {
      return companyServices
        ...
        .then($scope.saveMsg91Success, $scope.saveMsg91Failure);
    }
  };
  ...
  $scope.saveSendGrid = function() {
    ...
    if ($scope.sGrid.authKey.length > 0 && $scope.sGrid.subject.length > 0) {
      return companyServices
        ...
        .then($scope.saveSendGridSuccess, $scope.saveSendGridFailure);
    }
  };
  ...
  $scope.addfyYears = function() {
    ...
    return ($scope.fyYears = _.difference(
      $scope.fyYears,
      $scope.fy.addedFYears
    ));
  };
  ...
  $scope.getFY = () =>
    companyServices
      .getFY($rootScope.selectedCompany.uniqueName)
      .then($scope.getFYSuccess, $scope.getFYFailure);
  ...
  $scope.changeFyPeriod = function(period) {
    ...
    return companyServices
      ...
      .then($scope.changeFyPeriodSuccess, $scope.changeFyPeriodFailure);
  };
  ...
  $scope.changeFyPeriodSuccess = function(res) {
    ...
    $scope.fy.years = res.body.financialYears;
    $scope.setFYname($scope.fy.years);
    return $scope.getcurrentFYfromResponse(
      $rootScope.currentFinancialYear,
      $scope.fy.years
    );
  };
  ...
  $scope.getCurrentPeriod = function() {
    ...
    if (!_.isNull(cmp)) {
      let fromMonth = moment(
        cmp.activeFinancialYear.financialYearStarts,
        "DD/MM/YYYY"
      ).month();
      let toMonth = moment(
        cmp.activeFinancialYear.financialYearEnds,
        "DD/MM/YYYY"
      ).month();
      let selectedPeriod =
        $scope.months[fromMonth] + "-" + $scope.months[toMonth];
  ...
  $scope.switchFY = function(year) {
    ...
    return companyServices
      ...
      .then($scope.switchFYSuccess, $scope.switchFYFailure);
  };
  ...
  $scope.lockFY = function(year) {
    ...
    if (year.isLocked) {
      return companyServices
        ...
        .then($scope.lockFYSuccess, $scope.lockFYFailure);
    } else {
      return companyServices
        .unlockFY(reqParam, data)
        .then($scope.unlockFYSuccess, $scope.unlockFYFailure);
    }
  };
  ...
  $scope.addFY = function(newFy) {
    ...
    return companyServices
      .addFY(reqParam, data)
      .then($scope.addFYSuccess, $scope.addFYFailure);
  };
  ...
  $scope.getYodleeAccounts = function() {
    ...
    return userServices
      ...
      .then($scope.getAccountsSuccess, $scope.getAccountsFailure);
  };
    ...
    if ($scope.banks.linked.length < 1) {
      ...
      return _.each($scope.banks.linked, bank =>
        _.each(bank.accounts, function(card) {
          if (
            _.isNull(card.transactionDate) ||
            _.isUndefined(card.transactionDate)
          ) {
  ...
  $scope.fetchSiteList = function(str) {
    ...
    if (data.name.length > 1) {
      return userServices
        ...
        .then($scope.searchSiteSuccess, $scope.searchSiteFailure);
    }
  };
  ...
  $scope.selectBank = function(bank) {
    ...
    if (bank.yodleeSiteLoginFormDetailList.length > 1) {
      ...
      $scope.banks.components =
        bank.yodleeSiteLoginFormDetailList[0].componentList;
  ...
  $scope.submitForm = function(bankDetails) {
    ...
    userServices
      .addSiteAccount(reqBody, companyUniqueName)
      .then($scope.addSiteAccountSuccess, $scope.addSiteAccountFailure);
  ...
  $scope.addSiteAccountSuccess = function(res) {
    ...
    if (res.body.mfa) {
      ...
      switch (res.body.yodleeMfaResponse.fieldType) {
        ...
        case "QUESTIONS":
          $scope.banks.mfaForm =
            res.body.yodleeMfaResponse.fieldInfo.questionAns;
  ...
  $scope.addMfaAccount = function(bankData) {
    ...
    return userServices
      .verifyMfa(unqObj, newMfa)
      .then($scope.verifyMfaSuccess, $scope.verifyMfaFailure);
  };
    ...
    return _.each($scope.banks.linked, function(bank) {
      ...
      return _.each(linkedAccounts, function(lAcc) {
        if (lAcc.linkedAccount !== null) {
          ...
          return ($scope.AccountsListToLink = _.without(
            $scope.AccountsListToLink,
            _.findWhere($scope.AccountsListToLink, linked)
          ));
        }
      });
    });
  ...
  $scope.LinkGiddhAccountConfirm = function(acc) {
    ...
    return modalService
      ...
      .then($scope.LinkGiddhAccountConfirmed);
  };
  ...
  $scope.LinkGiddhAccountConfirmed = function(res) {
    ...
    return userServices
      .addGiddhAccount(companyUniqueName, $scope.banks.toLinkObj)
      .then(
        $scope.LinkGiddhAccountConfirmSuccess,
        $scope.LinkGiddhAccountConfirmFailure
      );
  };
  ...
  $scope.LinkGiddhAccountConfirmSuccess = function(res) {
    ...
    toastr.success(
      `Account linked successfully with ${linkAccData.linkedAccount.name}`
    );
    let companyUniqueName = {
      cUnq: $rootScope.selectedCompany.uniqueName
    };
  ...
  $scope.removeGiddhAccount = function(card) {
    ...
    return modalService
      .openConfirmModal({
        ...
        body: `Are you sure you want to unlink ${card.linkedAccount
          .uniqueName} ?`,
        ok: "Yes",
        cancel: "No"
      })
      .then($scope.removeGiddhAccountConfirmed);
  };
  ...
  $scope.removeGiddhAccountConfirmed = function() {
    ...
    return userServices
      ...
      .then(
        $scope.removeGiddhAccountConfirmedSuccess,
        $scope.removeGiddhAccountConfirmedFailure
      );
  };
  ...
  $scope.deleteAddedBank = function(card) {
    ...
    return modalService
      ...
      .then($scope.deleteAddedBankAccountConfirmed);
  };
  ...
  $scope.deleteAddedBankAccountConfirmed = function() {
    ...
    return userServices
      ...
      .then(
        $scope.deleteAddedBankAccountConfirmedSuccess,
        $scope.deleteAddedBankAccountConfirmedFailure
      );
  };
  ...
  $scope.refreshAccounts = function() {
    ...
    return userServices
      ...
      .then($scope.refreshAllSuccess, $scope.refreshAllFailure);
  };
  ...
  $scope.updateTransactionDate = function(date) {
    ...
    return userServices
      ...
      .then(
        $scope.updateTransactionDateSuccess,
        $scope.updateTransactionDateFailure
      );
  };
  ...
  $scope.changedate = function(date) {
    ...
    return modalService
      ...
      .then(() => $scope.updateTransactionDate(date));
  };
  ...
  $scope.connectBank = function() {
    if ($scope.linkedAccountsExist === false) {
      ...
      return userServices
        .connectBankAc($rootScope.selectedCompany.uniqueName)
        .then($scope.connectBankSuccess, $scope.connectBankFailure);
    }
  };
  ...
  $scope.reconnectAccount = function(account) {
    ...
    return userServices
      ...
      .then($scope.reconnectAccountSuccess, $scope.reconnectAccountFailure);
  };
  ...
  $scope.refreshToken = function(account) {
    ...
    return userServices
      ...
      .then($scope.refreshTokenSuccess, $scope.refreshTokenFailure);
  };
  ...
  $scope.$on(
    "$stateChangeStart",
    (event, toState, toParams, fromState, fromParams) => $scope.resetSteps()
  );
