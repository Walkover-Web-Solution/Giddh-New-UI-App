    ...
    module.factory('nzTour', function ($q, $rootScope, $compile, $timeout) {
        ...
        function doBefore(direction) {
            if (service.current.tour.steps[service.current.step].before) {
                return service.current.tour.steps[service.current.step].before(direction, service.current.step);
            }
            return $q.when(null);
        }
        ...
        function doAfter(direction) {
            if (service.current.tour.steps[service.current.step].after) {
                return service.current.tour.steps[service.current.step].after(direction, service.current.step);
            }
            return $q.when(null);
        }
    ...
    module.directive('nzTour', function ($q, $compile) {
        return {
            template: [
                ...
                '            <button id="nzTour-previous" ng-show="view.step > 0 && view.showPrevious" ng-click="previous($event)">{{view.previousText}}</button>',
                '            <button id="nzTour-next" ng-show="view.step >= 0 && view.showNext" ng-click="next($event)" class="success">{{view.nextText}}</button>',
                '        </div>',
                '    </div>',
                '</div>',
